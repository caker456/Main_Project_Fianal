...
def insert_dummy_user(db: PostgresDB, user_id: str, plain_password: str, user_role='R1', member_grade='G2', name='홍길동'):
    hashed_password = safe_bcrypt_hash(plain_password)

    conn = db.get_conn()
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT nextval('seq_member_id')")
        member_id = cursor.fetchone()[0]

        cursor.execute("""
            INSERT INTO member_info 
            (member_id, id, password, name, phone, email, member_role, member_grade)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
        """, (
            member_id, user_id, hashed_password, name,
